...
│   └── SettingSlider.qml, ChoiceRow.qml, DropdownRow.qml, ToggleRow.qml, WidgetRow.qml, GroupRow.qml, PathRow.qml, SearchEngineRow.qml, DefaultsRow.qml, PositionIcon.qml   # Rows of the settings panel
...
| Bar widgets (Positioning tab) | Each bar widget | on / off (check box), Left / Center / Right, and its place there | the original layout |
...
| OSD | Volume: distance from the edge (from the edges it is placed against; at the top or bottom on the bar's side, from the bar) | 0 – 400 px | 60 px |
| OSD | Caps Lock / Num Lock: position | as for the volume | Bottom center |
| OSD | Caps Lock / Num Lock: distance from the edge | 0 – 400 px | 60 px |
| Launcher | Default tab (the one the launcher opens on) | All / Applications / Files / Web | All |
| Launcher | Results shown (the most the list shows at once, the launcher shrinking to fewer; more scroll) | 3 – 20 | 7 |
...
- **The bell** is a bar widget like the others, just before the power button by default (a layout saved before it existed doesn't have it until you put it in a pill in the settings' Bar widgets category, or `settings place notifications right`). The bell is accent-colored while the center has notifications, and red while one of them is urgent. Their number is written to the right of the bell: it goes down only when one is dismissed, not when its pop-up goes away. A **right click** switches *do not disturb* on or off (also in the center's header and the settings' Notifications category): the bell is crossed out and no pop-up shows, except for urgent notifications; everything still goes to the center.
